...
#SBATCH --job-name=fmri_predictivity_models
#SBATCH --output=output/fmri_predictivity_models_%A_%a.out
#SBATCH --error=output/fmri_predictivity_models_%A_%a.err
#SBATCH --mem=16000
...
#SBATCH --array=0-1  # 0: robust, 1: standard
...
echo "=== Starting fMRI Predictivity Model Analysis ==="
echo "Current directory: $(pwd)"
echo "SLURM_ARRAY_TASK_ID: $SLURM_ARRAY_TASK_ID"
echo "================================================"

# Define the model mapping
# Array index 0: robust model
# Array index 1: standard model
MODEL_TYPES=("robust" "standard")

# Get the current job's model type
CURRENT_MODEL_TYPE=${MODEL_TYPES[$SLURM_ARRAY_TASK_ID]}

echo "Processing: Model=$CURRENT_MODEL_TYPE"

# Create a modified version of the main script that only processes one model
echo "=== Creating single-model script for $CURRENT_MODEL_TYPE ==="

# Create a temporary script that only processes the current model
cat > temp_run_single_model.py << EOF
import os
import subprocess
import sys
import h5py
...
# Settings - only process the current model
MODEL_TYPES = ['$CURRENT_MODEL_TYPE']  # Only the current model
NUM_SPLITS = 10
RANDSEED = 3882
OVERWRITE = True
...
print(f'\\nModel {CURRENT_MODEL_TYPE} completed at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
EOF

# Run the single-model script
echo "=== Running analysis for $CURRENT_MODEL_TYPE model ==="
python temp_run_single_model.py

# Clean up temporary script
rm temp_run_single_model.py

if [ $? -eq 0 ]; then
    echo "=== Analysis completed successfully for $CURRENT_MODEL_TYPE model ==="
else
    echo "=== ERROR: Analysis failed for $CURRENT_MODEL_TYPE model ==="
    exit 1
fi

echo "=== fMRI Predictivity Model Analysis Complete ==="
