...
def run_audio_metamer_generation(SIDX, LOSS_FUNCTION, INPUTAUDIOFUNCNAME, RANDOMSEED, 
                                 ...
                                 model_type='robust', duration=2, subclip_idx=None):
    ...
    base_filepath = os.path.join(MODEL_DIRECTORY, 'metamers/%s_%s/%d_SOUND_%s/'%(synth_name, timestamp, SIDX, ds.label_mapping[int(targ[0].cpu().numpy())]))
    try:
...
def main(raw_args=None):
    ...
    run_audio_metamer_generation(args.SIDX, args.LOSS_FUNCTION, args.INPUTAUDIOFUNCNAME, 
                                ...
                                args.MODEL_TYPE, args.duration, args.subclip_idx)  # Pass subclip_idx
