...
def run_audio_metamer_generation(SIDX, LOSS_FUNCTION, INPUTAUDIOFUNCNAME, RANDOMSEED, 
                                 ...
                                 duration=3, subclip_idx=None, debug_loss=False):
...
def main(raw_args=None):
    ...
    parser = argparse.ArgumentParser(description='Generate audio metamers')
    parser.add_argument('sound_idx', type=int, help='Index of the sound to use')
    parser.add_argument('-I', '--iterations', type=int, default=1000, help='Number of iterations')
    parser.add_argument('-N', '--num_rep_iter', type=int, default=1, help='Number of repetition iterations')
    parser.add_argument('-F', '--input_audio_func', type=str, default='natural_sounds_norman_haignere', help='Input audio function name')
    parser.add_argument('-L', '--loss_function', type=str, default='coarse_define_spectemp_inversion_loss_layer', help='Loss function to use')
    parser.add_argument('-D', '--model_directory', type=str, default=None, help='Model directory')
    parser.add_argument('--duration', type=float, default=3.0, help='Duration of audio in seconds')
    parser.add_argument('--subclip_idx', type=int, default=None, help='Subclip index')
    ...
    run_audio_metamer_generation(
        ...
        INPUTAUDIOFUNCNAME=args.input_audio_func,
        RANDOMSEED=42,  # Fixed random seed for reproducibility
        STRICT=True,
        overwrite_pckl=True,
        step_size=0.00001,
        NOISE_SCALE=1.0,
        ITERATIONS=args.iterations,
        NUMREPITER=args.num_rep_iter,
        OVERRIDE_SAVE=True,
        MODEL_DIRECTORY=args.model_directory,
        duration=args.duration,
        subclip_idx=args.subclip_idx,
